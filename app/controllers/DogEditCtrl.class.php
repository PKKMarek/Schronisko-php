    ...
    public function action_DogEdit()
    {
        ...
        if($this->validate()){
			...
			foreach ($behaviors as $key => $value) {
				...
				//	"BehaviorID" => $value
				//]);
				$this->behaviors[$key] = $select['Behavior types'];
			}
			
			$select = new SelectDog($this->form->dogID);//getDB()->get("Dog","*",[
			//	"idDog" => $this->form->dogID 
			//]);

			$this->form->dogName     = $select['Name'];
			$this->form->dogAge      = $select['Age'];
			$this->form->dogBreed    = $select['Breed'];
			$this->form->dogHairType = $select['Hair_Type'];
			$this->form->dogImage    = $select['img'];

			// $this->form->dogBehavior = $select[''];
    ...
    public function action_DogSave()
    {
        ...
        if($this->validate()){
            if ($this->form->dogID == "") {
				...
				foreach ($this->behaviors as $behaviorId){				
				
					new InsertBehavioralDog($this->dogID,$behaviorId);
